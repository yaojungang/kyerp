	...
	constructor : function(_cfg) {
		...
		org.kyerp.warehouse.MaterialPanel.superclass.constructor.call(this, {
					...
					items : [{
								...
								maxSize : 250,
								autoScroll : true,
								items : [this.materialCategoryTree]
							}, this.materialList]
				});
		...
		this.materialCategoryTree.on("click", function(node) {
			this.materialList.store.on('beforeload', function(thiz, options) {
					Ext.apply(thiz.baseParams, {mCategoryId : node.attributes.id});
				},this);
			this.materialList.store.load({params : {start:0,limit:20,mCategoryId : node.attributes.id}});
