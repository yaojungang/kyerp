...
public class CalSK{
	...
	public List<AfValuation> calSK(AfBase af) {
		...
		if(af != null) {
			...
			double CoverPressDanjia2 = 90; // 封面印刷系数2
			...
			if(afes != null) {
				...
				for (Iterator iterator = afes.iterator(); iterator.hasNext();) {
					...
					if(afe.getEType().equals("Cover")) { // 计算封面的费用
						Cover_ColorAmount = afe.getEColorBackN() + afe.getEColorFrontN();
						if("05".equals(afe.getEMachine()) || "双面".equals(afe.getEMachine())) {
							shangbanFee = 12;
						}
						...
						CoverPSban_Fee = (PSbanFee - shangbanFee) * Cover_ColorAmount;
						...
						CoverPSbanFee.setCalProcess(CoverPSban_danjia + "x" + Cover_ColorAmount);
						CoverPSbanFee.setTotalAmount(com.tyopf.util.MathTools.round(CoverPSban_Fee, 2));
						...
						CovershangbanFee.setCalProcess(shangbanFee + "x" + Cover_ColorAmount);
						CovershangbanFee.setTotalAmount(com.tyopf.util.MathTools.round(Covershangban_Fee, 2));
						bjList.add(CovershangbanFee);

						String CoverPressProcess = "";

						if(Cover_ColorAmount < 4 && Cover_ColorAmount > 0) {
							CoverPress_Fee = Cover_PressDanjia - CoverPressDanjia2 * (4 - Cover_ColorAmount);
							CoverPressProcess = Cover_PressDanjia + "-" + CoverPressDanjia2 + "x" + "(4-" + Cover_ColorAmount + ")";
						}
						...
						if(Cover_ColorAmount > 4) {
							CoverPress_Fee = Cover_PressDanjia + CoverPressDanjia2 * (Cover_ColorAmount - 4);
							CoverPressProcess = Cover_PressDanjia + "+" + CoverPressDanjia2 + "x" + "(" + Cover_ColorAmount + "-" + "4)";
						}
						// 超过5令,每增加1令增加100块钱
						double Paper_Ream = new Double(afe.getEReam());
						if(Paper_Ream > 5) {
							CoverPress_Fee = CoverPress_Fee + (Paper_Ream - 5) * 100;
							CoverPressProcess = CoverPressProcess + "+" + (Paper_Ream - 5) + "x" + 100;
						}
						...
						CoverPressbanFee.setTotalAmount(com.tyopf.util.MathTools.round(CoverPress_Fee, 2));
					...
					if(afe.getEType().equals("BB")) { // 计算正文的费用
						...
						if(PEdition_n > 2 && "-1".equals(PEdition.substring(PEdition_n - 2, PEdition_n))) {
							...
							BBpingbanFee.setCalProcess(BB_Plate_pk + "x" + BB_Plate_Amont);
							BBpingbanFee.setTotalAmount(com.tyopf.util.MathTools.round(BB_BPlate_fee, 2));
						...
						BB_PSFee.setCalProcess(BB_PS_fee + "x" + BB_Plate_Amont);
						BB_PSFee.setTotalAmount(com.tyopf.util.MathTools.round(BB_PS_Fee, 2));
						bjList.add(BB_PSFee);
						...
						BB_ShangFee.setCalProcess(BB_Shangban_fee + "x" + BB_Plate_Amont);
						BB_ShangFee.setTotalAmount(com.tyopf.util.MathTools.round(BB_Shang_Fee, 2));
						...
						ColorAmount = Cover_ColorAmount = afe.getEColorBackN() + afe.getEColorFrontN();
						;
						...
						BB_PrintFee.setCalProcess("" + EPS + "x" + KPA + "x" + BB_pk + "x" + pks + "x" + ColorAmount);
						BB_PrintFee.setTotalAmount(com.tyopf.util.MathTools.round(BB_Print_fee, 2));
					...
					if(afe.getEType().equals("CI")) { // 计算插页费用
						...
						CI_PS_Fee.setCalProcess(CI_PS_fee + "x" + CI_Plate_Amont);
						CI_PS_Fee.setTotalAmount(com.tyopf.util.MathTools.round(CI_PSFee, 2));
						...
						CI_ShangFee.setCalProcess(CI_shangPS_fee + "x" + CI_Plate_Amont);
						CI_ShangFee.setTotalAmount(com.tyopf.util.MathTools.round(CI_Shang_Fee, 2));
						...
						if(afe.getEColorFrontN() == 1) {
							...
							CI_PrintFee.setTotalAmount(com.tyopf.util.MathTools.round(CI_Print_Fee, 2));
						...
						if(afe.getEColorFrontN() == 4) {
							...
							double Paper_Ream = new Double(afe.getEReam());
							if(Paper_Ream > 5) {
								CI_Print_Fee = CI_Print_Fee + (Paper_Ream - 5) * 100;
								CIPressProcess = CIPressProcess + "+" + (Paper_Ream - 5) + "x" + 100;
							}
				...
				for (Iterator iterator1 = afds.iterator(); iterator1.hasNext();) {
					...
					if("本厂".equals(afd.getAfDFactory())) {
						if(afd.getAfDItem().equals("胶订")) {
							...
							BB_BindingFee.setCalProcess("" + binding_pks + "x" + KPA + "x" + "1000" + "x" + EPS);
							BB_BindingFee.setTotalAmount(com.tyopf.util.MathTools.round(Binding_fee, 2));
						...
						if(afd.getAfDItem().equals("骑马订")) {
							...
							BB_BindingFee.setChejian("装订");
							BB_BindingFee.setDanjia(binding_pks);
							BB_BindingFee.setCalProcess("" + qmdDanjia + "x" + KPA + "x1000");
							BB_BindingFee.setTotalAmount(com.tyopf.util.MathTools.round(Binding_fee, 2));
							bjList.add(BB_BindingFee);
						}
					}
