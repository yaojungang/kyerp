...
public class GenericDAOImpl<T> extends JpaDaoSupport implements GenericDAO<T> {
	...
	public T getByName(final String propertyName, final Object value) {
		List<T> ret = (List<T>) this.getJpaTemplate().execute(new JpaCallback() {
			public Object doInJpa(EntityManager em) throws PersistenceException {
				StringBuffer sb = new StringBuffer("select object form ");
				sb.append(clz.getName());
				sb.append("obj").append("where").append("obj.").append(propertyName).append("=:value");
				System.out.println("运行的SQL：" + sb.toString());
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<T> query(final String sql, final Object[] paras, final int begin, final int count) {
		...
		list = (List<T>) this.getJpaTemplate().execute(new JpaCallback() {
			public Object doInJpa(EntityManager em) throws PersistenceException {
				...
				str.append(sql);
				System.out.println("运行的SQL：" + str.toString());
