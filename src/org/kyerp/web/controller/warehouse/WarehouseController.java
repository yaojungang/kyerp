...
public class WarehouseController extends BaseController {
	...
	public String list(Long parentId, Integer start, Integer limit, Model model) {
		...
		// set parent id
		if (null != parentId) {
			wherejpql.append(" and parentWarehouse.id=?").append(
					queryParams.size() + 1);
	...
	public String tree(Model model) {
		LinkedHashMap<String, String> orderby = new LinkedHashMap<String, String>();
		orderby.put("id", "asc");
		QueryResult<Warehouse> queryResult = warehouseService
				.getScrollData(orderby);
		List<ExtTreeNode> extTreeList = new ArrayList<ExtTreeNode>();
		if (queryResult.getResultlist().size() == 0) {
			Warehouse warehouse = new Warehouse();
			warehouse.setName("库房");
			warehouseService.save(warehouse);
			model.addAttribute("jsonText", "[{id:1,text:'库房',leaf:true}]");
		} else {
			for (Warehouse d : queryResult.getResultlist()) {
				ExtTreeNode node = new ExtTreeNode();
				node.setId(new Integer(d.getId().toString()));
				node.setText(d.getName());
				if (null != d.getParentWarehouse()
						&& d.getParentWarehouse().getId() > 0) {
					node.setParentId(new Integer(d.getParentWarehouse().getId()
							.toString()));
				}
				if (d.getId() == 1) {
	...
	public String save(WarehouseExtGridRow warehouseRow, ModelMap model) {
		...
		if (warehouseRow.getParentWarehouseId() != 0) {
			warehouse.setParentWarehouse(warehouseService.find(warehouseRow
