...
public class MaterialController {
	...
	public String list(Long mCategoryId, Integer start, Integer limit,
		...
		if (null != mCategoryId && mCategoryId > 0) {
			...
			jpql.append(" materialCategory.id=?").append((params.size() + 1));
			params.add(mCategoryId);
		}
		LinkedHashMap<String, String> orderby = new LinkedHashMap<String, String>();
		orderby.put("id", "asc");
		QueryResult<Material> queryResult = materialService.getScrollData(
				start, limit, jpql.toString(), params.toArray(), orderby);
		...
		for (Material m : queryResult.getResultlist()) {
			...
			if (null != m.getMaterialBatchs()
					&& m.getMaterialBatchs().size() > 0) {
				...
				for (int i = 0; i < m.getMaterialBatchs().size(); i++) {
					...
					batchNames[i] = m.getMaterialBatchs().get(i)
							.getBatchNumber();
				}
			...
			rr.setPrice(new Double(m.getPrice().toString()));
	...
	public String save(MaterialExtGridRow materialRow, ModelMap model) {
		...
		if (null != materialRow.getMaterialCategoryId()) {
			material.setMaterialCategory(materialCategoryService
					.find(materialRow.getMaterialCategoryId()));
		}
		...
		if (null != materialRow.getSupplierId()) {
			material.setSupplier(supplierService.find(materialRow
					.getSupplierId()));
		}
		if (null != materialRow.getWarehouseId()) {
			material.setWarehouse(warehouseService.find(materialRow
					.getWarehouseId()));
		}
		...
		long id = material.getId() > 0 ? material.getId() : materialService
				.findLast().getId();
