...
/** ***************************************************************************** */
org.kyerp.warehouse.UnitStore = new Ext.data.Store({
			autoLoad : true,
			proxy : new Ext.data.HttpProxy({
						url : org.kyerp.warehouse.UnitPanel_LIST_URL
					}),
			reader : new Ext.data.JsonReader({
						...
						id : "id"
							...
							'serialNumber', 'note', 'childUnitIds',
							'childUnitNames', 'parentUnitId',
							'parentUnitName'])
		});
/** ***************************************************************************** */
org.kyerp.warehouse.UnitTree = new Ext.tree.TreePanel({
	title : '计量单位',
	loader : new Ext.tree.TreeLoader({
				dataUrl : org.kyerp.warehouse.UnitPanel_TREE_URL
			}),
	root : {
		nodeType : 'async',
		id : 'root',
	...
	tools : [{
				...
				qtip : '刷新',
				handler : function() {
					org.kyerp.warehouse.UnitTree.getRootNode().reload();
				}
			}],
	rootVisible : false,
	border : false,
...
/** ***************************************************************************** */
org.kyerp.warehouse.UnitGrid = new Ext.grid.EditorGridPanel({
			title : '计量单位资料',
			store : org.kyerp.warehouse.UnitStore,
...
/** ***************************************************************************** */
org.kyerp.warehouse.UnitPanel = Ext.extend(Ext.Panel, {
	tree : null,
	grid : null,
	constructor : function(_cfg) {
		Ext.apply(this, _cfg);
		this.tree = org.kyerp.warehouse.UnitTree;
		this.grid = org.kyerp.warehouse.UnitGrid;
		...
		this.tree.on("click", function(node) {
					node.expand();
					node.select();
					var store = org.kyerp.warehouse.UnitStore;
		...
		this.grid.on('afteredit', function(e) {
					...
					Ext.Ajax.request({
								...
								success : function() {
									...
									var node = org.kyerp.warehouse.UnitTree
											.getSelectionModel()
	...
	onDeleteUnitClick : function(_btn) {
		if (_btn == "yes") {
			this.onDeleteUnit();
		}
	},
		...
		try {
			...
			Ext.Ajax.request({
						...
						success : function(response) {
							var data = Ext.decode(response.responseText);
							if (data.success) {
								var snode = org.kyerp.warehouse.UnitTree
								...
								org.kyerp.warehouse.UnitGrid.getStore()
										.reload();
								Ext.MessageBox.alert('警告', '删除计量单位资料完成。');
							} else {
								Ext.MessageBox.alert('警告', data.msg);
							}
						}
...
Ext.extend(org.kyerp.module, {
			init : function() {
				this.body = new org.kyerp.warehouse.UnitPanel({
							border : false,
							bodyBorder : false
						});
